...
MANIFEST_JSON="$WALLET_DIR/manifest.json"
...
if [[ -z "$current_package_version" || -z "$current_manifest_version" ]]; then
  echo "Failed to read current version from wallet package/manifest."
  exit 1
fi
...
echo "Current Wallet version: $current_package_version"
printf "Next Wallet version: "
read -r next_version
...
tag_name="wallet-v$next_version"
...
echo "Building Wallet release artifacts..."
(cd "$WALLET_DIR" && npm run pack)
(cd "$WALLET_DIR" && npm run pack:discovery)
